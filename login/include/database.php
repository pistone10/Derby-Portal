...
class MySQLDB
{
    ...
    */
   function addNewUser($username, $password, $email, $name, $derby, $phone, $token, $usersalt){
      $time = time();
      $config = $this->getConfigs();
     ...
     $query = "INSERT INTO ".TBL_USERS." SET username = :username, password = :password, name = :name, Derby_Name = :derby, Phone = :phone, usersalt = :usersalt, userid = 0, userlevel = $ulevel, email = :email, timestamp = $time, actkey = :token, ip = '$userip', regdate = $time";
     $stmt = $this->connection->prepare($query);
     return $stmt->execute(array(':username' => $username, ':password' => $password, ':name' => $name, ':derby' => $derby, ':phone' => $phone, ':usersalt' => $usersalt, ':email' => $email, ':token' => $token));
   }
